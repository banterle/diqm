...
def train(loader, model, optimizer, args):
    ...
    for src, dst, q, lmax in progress:
...
if __name__ == '__main__':
    ...
    run_name = 'q_{0[dataset]}_lr{0[lr]}_e{0[epochs]}_b{0[batch]}_en{0[encoding]}'.format(params)
    ...
    train_data = HdrVdpDataset(train_data, args.data, bScaling = args.scaling, encoding = args.encoding)
    ...
    val_data = HdrVdpDataset(val_data, args.data, bScaling = args.scaling, encoding = args.encoding)
    ...
    test_data = HdrVdpDataset(test_data, args.data, bScaling = args.scaling, encoding = args.encoding)
    test_loader = DataLoader(test_data, shuffle=False, batch_size=1, num_workers=8, pin_memory=True)
    
    model = QNet(6, 1, params_size = None)
